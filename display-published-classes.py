""" This script displays the published classes in a given semester

    It is my understanding that these are the semester IDs
    `enrollment_term_id` is the value we will be in the URL
        - Winter 2018: 49
        - Fall 2018: 50
        - Spring 2018: 51
        - Summer 2018: 52
...
# Canvas API Key
API_KEY = input('Type your API Key below and hit [ENTER]\n')
...
with open(f'{term_id}-{term_id_dict[term_id][0]}-published-course-export.csv',
          'w') as f:
    print('course_id', 'course_name', 'course_link', 'state', sep=',', file=f)
    for k, v, l in c_course_ids:
        c = f'https://byu.instructure.com/courses/{k}'
        print(k, v, c, l, sep=',', file=f)
